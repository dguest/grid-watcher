...
cat <(date) \
    ...
    <(pandamon -u 'Andrea Matic' group.phys-exot*EXOT27 -j |\
          jq '[.[] | {"id": (.taskname | split(".")[3]), "name": (.taskname | split(".")[4]), "fullname": .taskname, "done": .dsinfo.pctfinished, "failed": .dsinfo.pctfailed, "input":.datasets[] | select(.type == "input") | .containername}]' |\
